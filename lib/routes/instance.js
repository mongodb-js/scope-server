...
var ReadPreference = require('mongodb/node_modules/mongodb-core').ReadPreference;
var OPTIONS = {
  ...
  getAllDatabaseNames(db, function(err, names) {
    ...
    var tasks = names.map(function(name) {
      return function(cb) {
        ...
        db.db(name)
          .listCollections({}, OPTIONS) // @note: Need to pass readPreference in OPTIONS for 3.x
  ...
  db.admin().command({
    ...
    if (err) {
      if (mongodbError.isNotAuthorized(err)) {
        debug('hostInfo unavailable for this user');
...
function getBuild(db, fn) {
  db.admin().buildInfo(function(err, data) {
    ...
    fn(null, {
      ...
      enterprise_module: data.modules && data.modules.indexOf('enterprise') !== -1
    });
  });
}
