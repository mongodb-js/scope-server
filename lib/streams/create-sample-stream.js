...
module.exports = function createSampleStream(db, collection_name, opts) {
  ...
  module.exports.hasNativeSampling(db, function(err, hasNativeSampling) {
    debug('hasNativeSampling returned', err, hasNativeSampling);
  });
  return $sampleShim(db, collection_name, opts);
};

module.exports.hasNativeSampling = function(db, fn) {
  getKernelVersion(db, function(err, version) {
    if (err) return fn(err);
    fn(null, semver.gte(version, '3.1.6'));
  });
};
