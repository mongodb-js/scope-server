...
Replicaset.prototype.state = function(fn) {
  async.parallel({
    ...
    res.config.members.map(function(opts) {
      memberOpts[toURL(opts.host).getId()] = opts;
    });

    state.members = res.rs.members.map(function(member) {
      var instance_id = toURL(member.name).getId();
      var opts = memberOpts[instance_id];
      var doc = {
...
module.exports.discover = function(db, fn) {
  ...
  add(_.keys(state.secondaries).map(function(hp) {
    return toURL(Instance.getId(hp)).rs(rs).state('secondary');
  }));

  add(_.keys(state.arbiters).map(function(hp) {
    return toURL(Instance.getId(hp)).rs(rs).type('arbiter');
  }));

  add(_.keys(state.passives).map(function(hp) {
    return toURL(Instance.getId(hp)).rs(rs).type('passive');
  }));

  add(toURL(Instance.getId(state.primary.name)).rs(rs).state('primary'));
  debug('discovered `%j`', {
    instances: instances,
    type: 'replicaset',
    replicaset: rs
