...
function mount(tokenData, ctx, next) {
  ...
  Deployment.get(tokenData.deployment_id, function(err, deployment) {
    ...
    if (ctx.instance_id) {
      ...
      if (!ctx.instance) {
        var msg = format(
          'Tried getting a connection to `%s` but it is not in deployment `%s`',
          ctx.instance_id, tokenData.deployment_id);
...
module.exports.create = function(model, fn) {
  Session.create(model, function(err, session) {
    var d = {
      deployment_id: model.deployment._id,
    ...
    var res = {
      ...
      token: token,
      deployment_type: model.deployment.type,
      deployment_id: model.deployment._id,
