...
var debug = require('debug')('scout-server:models:token');

var Deployment = require('./deployment');
var Session = require('./session');

/**
 * @todo (imlucas): add an explanation to this file about how
 * tokens work in general here. The code in this file does
 * seem pretty straightfoward but the token system
 * isn't so straightforward to someone who hasn't used it before.
 */
...
function mount(tokenData, ctx, next) {
  ...
  Deployment.get(tokenData.deployment_id, function(err, deployment) {
    ...
    Session.get(tokenData.session_id, function(err, connection) {
      if (err) {
        return next(err);
      }
...
module.exports.create = function(model, fn) {
  Session.create(model, function(err, session) {
    if (err) {
      return fn(err);
    }
