    ...
    self.getShards(function(err, shards) {
      ...
      self.db.collection('mongos').find({}).toArray(function(err, docs) {
        ...
        docs.map(function(doc) {
          self.data.instances.push(toURL(doc._id).type('router').toJSON());
        });
        ...
        self.getConfigServers(function(err, urls) {
          ...
          urls.map(function(url) {
            self.data.instances.push(toURL(url).type('config').toJSON());
          });
...
Sharding.prototype.getChunks = function(done) {
  this.db.collection('chunks').find({}).sort({
    ...
    if (err) return done(err);
      ...
      var lastmodUnix = doc.lastmodEpoch.getTimestamp().getTime();
      return {
        _id: doc._id,
        last_modified_on: new Date(lastmodUnix + doc.lastmod.high_ * 1000),
