    ...
    if (attrs.gssapi_service_name) {
      if (attrs.auth_mechanism !== 'GSSAPI') {
        return new TypeError('The `gssapi_service_name` field does not apply when using'
          + attrs.auth_mechanism + '.');
      }
    }

    if (!attrs.ssl) {
      if (attrs.ssl_validate) {
        return new TypeError('The `ssl_validate` requires `ssl = true`');
      }
      if (attrs.ssl_ca) {
        return new TypeError('The `ssl_ca` requires `ssl = true`');
      }
      if (attrs.ssl_cert) {
        return new TypeError('The `ssl_cert` requires `ssl = true`');
      }
      if (attrs.ssl_key) {
        return new TypeError('The `ssl_key` requires `ssl = true`');
      }
      if (attrs.ssl_pass) {
        return new TypeError('The `ssl_vpass` requires `ssl = true`');
      }
    }
  },
