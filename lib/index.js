...
/**
 * @todo (imlucas) Switch to `mongodb-js-config`.
 */
...
/* eslint handle-callback-err: 0, no-shadow:0, no-console:0 */
app.use(function(err, req, res) {
  mongodbError.decode(err, function(err) {
    // @todo (imlucas): `metrics.trackError(err)`
    bugsnag.notify(err, bugsnag.requestInfo(req));

    if (!err.isBoom) {
      if (err.command) {
        console.error('Driver error running command `%s`:', err.command, err.stack);
      } else {
    ...
    if (process.env.NODE_ENV !== 'production' || err.driver) {
