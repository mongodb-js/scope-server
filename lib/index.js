...
module.exports.start = function(opts, fn) {
  ...
  app.server.on('error', function(err) {
    if (err.code === 'EADDRINUSE') {
      // @todo (imlucas): Send a GET to `/api/v1`,
      // [turn on the sirens][minion-siren-gif] and die hard if:
      // - `res.statusCode !== 200`: something else using the port
      // - `res.body.version !== pkg.version`: bad things prolly gon' happen...
      // [minion-siren-gif]: http://media.giphy.com/media/wuOtkQMVrqdRS/giphy.gif
      console.error('  another scout-server is already running on %s',
        app.config.get('listen'));
    }
  ...
  app.server.listen(app.config.get('port'), app.config.get('hostname'), function() {
    console.log('   ');
    console.log('   scout-server v%s listening on %s',
      pkg.version, app.config.get('listen'));
    console.log('   ');
    if (fn) {
      fn(null, app);
    }
